!function (e, n, t, r) {
...
}("analytics.meituan.com", "stable", "Analytics", ""), Analytics("config", "appnm", "movie"), Analytics("config", "alwaysSendReferrer", !0), Analytics("use", "movie"), Analytics("set", "uid", window.system ? system.uid : ""), function (e) {
    ...
    function g(e) {
        if (e.url) {
            var n, t = {filename: e.url, lineno: e.line, colno: e.column, "function": e.func || "?"}, r = m(e);
            if (r) {
                var i = ["pre_context", "context_line", "post_context"];
                for (n = 3; n--;)t[i[n]] = r[n]
            }
            return t.in_app = !(P.includePaths.test && !P.includePaths.test(t.filename) || /(Raven|TraceKit)\./.test(t["function"]) || /raven\.(min\.)?js$/.test(t.filename)), t
        }
    }

    function m(e) {
        if (e.context && P.fetchContext) {
    ...
    function d(e, n, t, r, i, o) {
        var a, s;
        P.ignoreErrors.test && P.ignoreErrors.test(n) || (n += "", n = x(n, P.maxMessageLength), s = e + ": " + n, s = x(s, P.maxMessageLength), i && i.length ? (t = i[0].filename || t, i.reverse(), a = {frames: i}) : t && (a = {
                frames: [{
                    filename: t,
                    lineno: r,
                    in_app: !0
                }]
            }), P.ignoreUrls.test && P.ignoreUrls.test(t) || P.whitelistUrls.test && !P.whitelistUrls.test(t) || b(v({
            exception: {
                type: e,
                value: n
    ...
    function b(e) {
        var n = {project: I, logger: P.logger, platform: "javascript"}, r = w();
        r && (n.request = r), e = v(n, e), e.tags = v(v({}, P.tags), e.tags), e.extra = v(v({}, P.extra), e.extra), e.extra = v({"session:duration": y() - J}, e.extra), u(e.tags) && delete e.tags, M && (e.user = M), P.release && (e.release = P.release), a(P.dataCallback) && (e = P.dataCallback(e) || e), e && !u(e) && (a(P.shouldSendCallback) && !P.shouldSendCallback(e) || (A = e.event_id || (e.event_id = O()), C("debug", "Raven about to send:", e), k() && (P.transport || E)({
            url: N,
    ...
    function k() {
        return !!D && (!!N || (K || C("error", "Error: Raven has not been configured."), K = !0, !1))
    }
    ...
    function C(e) {
        H[e] && X.debug && H[e].apply(B, L.call(arguments, 1))
    }
    ...
    var j = {remoteFetching: !1, collectWindowErrors: !0, linesOfContext: 7, debug: !1}, L = [].slice, $ = "?";
    j.wrap = function (e) {
        function n() {
            try {
        ...
        function o(e, n) {
            ...
            if (!n || j.collectWindowErrors) {
                for (var r in g)if (f(g, r))try {
                    g[r].apply(null, [e].concat(L.call(arguments, 2)))
                } catch (i) {
                    t = i
                }
                if (t)throw t
        ...
        function u() {
            var e = v, n = m;
            m = null, v = null, d = null, o.apply(null, [e, !1].concat(n))
        }
        ...
        function l(n, t) {
            ...
            if (v = i, d = n, m = r, e.setTimeout(function () {
                    d === n && u()
                }, i.incomplete ? 2e3 : 0), t !== !1)throw n
        }

        var p, h, g = [], m = null, d = null, v = null;
        ...
        function p(n) {
            for (var t, r, i, o, s = [e.location.href], l = document.getElementsByTagName("script"), f = "" + n, p = /^function(?:\s+([\w$]+))?\s*\(([\w\s,]*)\)\s*\{\s*(\S[\s\S]*\S)\s*\}\s*$/, h = /^function on([\w$]+)\s*\(event\)\s*\{\s*(\S[\s\S]*\S)\s*\}\s*$/, g = 0; g < l.length; ++g) {
                var m = l[g];
                m.src && s.push(m.src)
            }
            if (i = p.exec(f)) {
            ...
            if (!o(e.stack) && e.stack) {
                for (var n, t, a = /^\s*at (.*?) ?\(?((?:(?:file|https?|chrome-extension):.*?)|<anonymous>):(\d+)(?::(\d+))?\)?\s*$/i, s = /^\s*(.*?)(?:\((.*?)\))?@((?:file|https?|chrome).*?):(\d+)(?::(\d+))?\s*$/i, c = /^\s*at (?:((?:\[object object\])?.+) )?\(?((?:ms-appx|http|https):.*?):(\d+)(?::(\d+))?\)?\s*$/i, u = e.stack.split("\n"), f = [], p = /^(.*) is undefined$/.exec(e.message), h = 0, g = u.length; h < g; ++h) {
                    if (n = s.exec(u[h])) t = {
                        ...
                        t = {url: n[2], func: n[1] || $, line: +n[3], column: n[4] ? +n[4] : null}
                    }
        ...
        function m(n) {
            ...
            var a, s, l, p, h = /^\s*Line (\d+) of linked script ((?:file|https?)\S+)(?:: in function (\S+))?\s*$/i,
                g = /^\s*Line (\d+) of inline#(\d+) script in ((?:file|https?)\S+)(?:: in function (\S+))?\s*$/i,
                m = /^\s*Line (\d+) of function script\s*$/i, d = [], v = document.getElementsByTagName("script"),
                x = [];
            for (s in v)f(v, s) && !v[s].src && x.push(v[s]);
            for (s = 2, l = o.length; s < l; s += 2) {
                var y = null;
                if (a = h.exec(o[s])) y = {url: a[2], func: a[3], line: +a[1]}; else if (a = g.exec(o[s])) {
                    y = {url: a[3], func: a[4]};
                    var w = +a[1], b = x[a[2] - 1];
                    if (b && (p = t(y.url))) {
                        p = p.join("\n");
                    ...
                    var S = e.location.href.replace(/#.*$/, ""), k = a[1], _ = new RegExp(c(o[s + 1]));
                    p = u(_, [S]), y = {url: S, line: p ? p.line : k, func: ""}
                }
                if (y) {
        ...
        function v(e, n) {
            for (var t, i, o, a = /function\s+([_$a-zA-Z\xA0-\uFFFF][_$a-zA-Z0-9\xA0-\uFFFF]*)?\s*\(/i, s = [], c = {}, u = !1, f = v.caller; f && !u; f = f.caller)if (f !== x && f !== j.report) {
                ...
                if (o = p(f)) {
                    i.url = o.url, i.line = o.line, i.func === $ && (i.func = r(i.url, i.line));
                    var g = / '([^']+)' /.exec(e.message || e.description);
                    g && (i.column = l(g[1], o.url, o.line))
                }
                c["" + f] ? u = !0 : c["" + f] = !0, s.push(i)
            }
            n && s.splice(0, n);
            var m = {name: e.name, message: e.message, url: document.location.href, stack: s};
    ...
    var U, A, N, M, F, I, q = e.Raven, D = !("object" != typeof JSON || !JSON.stringify), P = {
        logger: "javascript",
        ignoreErrors: [],
        ignoreUrls: [],
        whitelistUrls: [],
    ...
    var X = {
        VERSION: "1.1.22", debug: !0, noConflict: function () {
            ...
            return n && p(n, function (e, n) {
                P[e] = n
            }), P.ignoreErrors.push(/^Script error\.?$/), P.ignoreErrors.push(/^Javascript error: Script error\.? on line 0$/), P.ignoreErrors = _(P.ignoreErrors), P.ignoreUrls = !!P.ignoreUrls.length && _(P.ignoreUrls), P.whitelistUrls = !!P.whitelistUrls.length && _(P.whitelistUrls), P.includePaths = _(P.includePaths), F = t.user, I = t.path.substr(r + 1), N = "//" + t.host + (t.port ? ":" + t.port : "") + "/" + o + "api/" + I + "/store/", t.protocol && (N = t.protocol + ":" + N), P.fetchContext && (j.remoteFetching = !0), P.linesOfContext && (j.linesOfContext = P.linesOfContext), j.collectWindowErrors = !!P.collectWindowErrors, X
        }, install: function () {
            return k() && !z && (j.report.subscribe(h), z = !0), X
            ...
            if (!P.ignoreErrors.test || !P.ignoreErrors.test(e))return b(v({message: e + ""}, n)), X
            ...
            return P.release = e, X
        }, setDataCallback: function (e) {
            return P.dataCallback = e, X
        }, setShouldSendCallback: function (e) {
            return P.shouldSendCallback = e, X
